...
create database dev_db; -- 새로운 데이터베이스(dev_db / 이름 자유)생성
show databases;
use dev_db; -- 데이터베이스로 들어감
...
create table person(
    ...
    comm numeric(10),
    dno int,
    foreign key(dno) references division(dno) -- FK 제약조건은 아래에 따로 명시해두지않으면 foreign key가 실행되지않음.
);
...
-- 참고 select 무엇을? / from 어디서? / where 조건은? / order by 정렬하는 기준은?

-- 1. 사번, 이름, 급여를 출력
select pno 사번, pname 이름, sal 급여 
from person;
...
-- 7. 사번, 이름, 상사이름
select w.pno, w.pname, m.pname as manager_name
from person w, person m
where w.manager = m.pno;

-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) – oracle과 다른 문법
select w.pno, w.pname, ifnull(m.pname, '★CEO★') as manager_name
from person w left outer join person m
on w.manager = m.pno;

-- 8-1 사번, 이름, 상사사번(상사가 없으면 ceo로 출력. ifnull함수의 매개변수의 타입이 상이해도 상관없음) – oracle과 다른 문법
select w.pno, w.pname, ifnull(m.pno, 'ceo') mno
from person w left outer join person m
on w.manager = m.pno;

-- 8-2 사번, 이름, 급여, 고액연봉자 or 일반연봉자
select pno, pname, if(sal>=3500, '고액연봉자', '일반연봉자') from person;
...
left join person m on w.manager=m.pno; -- 상사자 없는 사람도 출력alter
...
select pname, date_format(hiredate, '%Y년%m월%d일 %p %h시%i분%s초') from person;
